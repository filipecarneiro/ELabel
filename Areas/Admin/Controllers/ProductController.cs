...
namespace ELabel.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, [Bind("Id,Name,Volume,WineInformation,ProductIngredients,PackagingGases,NutritionInformation,ResponsibleConsumption,Certifications,Country,Sku,Ean")] WineProductEditDto wineProductEditDto)
        {
        ...
        public async Task<IActionResult> Import([Bind("File")] ImportFileUpload importFileUpload)
        {
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                foreach (ProductExcelDto importedProduct in importedProducts)
                {
                    ...
                    if(importedProduct.Ingredients != null)
                    {
                        ...
                        short lastOrder = 0;
                        foreach (IngredientWithOrderExcelDto ingredientWithOrderExcelDto in importedProduct.Ingredients)
                        {
